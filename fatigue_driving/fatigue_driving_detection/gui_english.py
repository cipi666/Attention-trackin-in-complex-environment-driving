...
class WinGUI(Tk):
    ...
    def __win(self):
        self.title("Object detection system")
        # 设置窗口大小、居中
        width = 519
        height = 680
    ...
    def __tk_button_upload_image(self, parent):
        btn = Button(parent, text="UploadImage", takefocus=False, )
        btn.place(x=62, y=29, width=95, height=30)
        return btn

    def __tk_button_video(self, parent):
        btn = Button(parent, text="UploadVideo", takefocus=False, )
        btn.place(x=162, y=29, width=95, height=30)
        return btn

    def __tk_button_cam(self, parent):
        btn = Button(parent, text="OpenCamera", takefocus=False, )
        btn.place(x=263, y=29, width=95, height=30)
        return btn
    ...
    def __tk_button_close(self, parent):
        btn = Button(parent, text="CloseVideoCam", takefocus=False, )
        btn.place(x=364, y=29, width=112, height=30)
        return btn

    def __tk_label_time(self, parent):
        label = Label(parent, text="Time：", anchor="center", )
    ...
    def __tk_label_count(self, parent):
        label = Label(parent, text="Count：", anchor="center", )
    ...
    def __tk_label_conf(self, parent):
        label = Label(parent, text="Conf：", anchor="center", )
    ...
    def __tk_label_class(self, parent):
        label = Label(parent, text="Class：", anchor="center", )
    ...
    def __tk_label_select_obj(self, parent):
        label = Label(parent, text="Select_Object：", anchor="center", )
        label.place(x=384, y=488, width=95, height=30)
        return label
    ...
    def __tk_label_driving_flag(self, parent):
        label = Label(parent, text="Driver status：", anchor="center", )
        label.place(x=41, y=585, width=86, height=31)
        return label

    def __tk_label_driving_show(self, parent):
        label = Label(parent, text="normal", anchor="center", borderwidth=1, relief="groove")
...
class Controller:
    ...
    def __init__(self):
        # todo 修改模型权重路径
        model_path = '/home/public/fatigue_driving/fatigue_driving_detection/runs/detect/train/weights/best.pt'
    ...
    def upload_image(self, evt):
        ...
        if self.image_path:
            ...
            if 'closed_eye' in detected_names or 'open_mouth' in detected_names:
                self.ui.tk_label_driving_show.config(text="Fatigue driving", background="red", foreground="black")
            else:
                self.ui.tk_label_driving_show.config(text='Normal driving', background="white", foreground="black")
    ...
    def upload_video(self, evt):
        ...
        """开启视频文件检测事件"""
        self.video_path = filedialog.askopenfilename(title="choose video file", filetypes=[("视频文件", "*.mp4 *.avi")])
        if self.video_path:
